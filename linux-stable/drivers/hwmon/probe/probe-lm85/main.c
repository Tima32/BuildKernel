...
struct lm85_data {
	...
	u8 cfg3;		/* Config Register 3 on ADT7463 */
...
static struct lm85_data *lm85_update_device(struct device *dev)
{
	...
	if (!data->valid ||
		...
		data->cfg3 = lm85_read_value(client, ADT7463_REG_CFG3);
...
static ssize_t show_fan_measure(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int t = 0;
	int nr = to_sensor_dev_attr(attr)->index;
...
static ssize_t set_fan_measure(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	int nr = to_sensor_dev_attr(attr)->index;
	struct lm85_data *data = dev_get_drvdata(dev);
	...
	if(val == 0)
	    data->cfg3 &= ~(1 << (nr + 4));
	lm85_write_value(client, ADT7463_REG_CFG3, data->cfg3);
	mutex_unlock(&data->update_lock);
	return count;
}
...
static SENSOR_DEVICE_ATTR(fan##offset##_measure, S_IRUGO | S_IWUSR,	\
		show_fan_measure, set_fan_measure, offset - 1)
...
/* ADT7463 specific registers */
#define ADT7463_REG_CFG2 0x73

static ssize_t show_ain(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int nr = to_sensor_dev_attr(attr)->index;
	int value = 0;
	...
	mutex_lock(&data->update_lock);
	value = lm85_read_value(client, ADT7463_REG_CFG2);
	mutex_unlock(&data->update_lock);

	return sprintf(buf, "%d\n", (value >> nr) & 0x1);
}

static ssize_t set_ain(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int nr = to_sensor_dev_attr(attr)->index;
	struct lm85_data *data = dev_get_drvdata(dev);
	...
	long val;
	int err;
	volatile long unsigned int tmp = 0;
	...
	mutex_lock(&data->update_lock);
	tmp = lm85_read_value(client, ADT7463_REG_CFG2);
	mutex_unlock(&data->update_lock);
	...
	mutex_lock(&data->update_lock);
	lm85_write_value(client, ADT7463_REG_CFG2, tmp);
	mutex_unlock(&data->update_lock);
	return count;
}

#define ain_reg(offset)							\
static SENSOR_DEVICE_ATTR(adt7463_ain##offset##_val,			\
		S_IRUGO | S_IWUSR, show_ain, set_ain, offset);
...
#define ADT7463_REG_FANPPR 0x7b

static ssize_t show_fanppr(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int nr = to_sensor_dev_attr(attr)->index;
	int value = 0;
	...
	mutex_lock(&data->update_lock);
	value = lm85_read_value(client, ADT7463_REG_FANPPR);
	mutex_unlock(&data->update_lock);

	return sprintf(buf, "%d\n", (value >> (nr * 2)) & 0x3);
}

static ssize_t set_fanppr(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int nr = to_sensor_dev_attr(attr)->index;
	struct lm85_data *data = dev_get_drvdata(dev);
	...
	mutex_lock(&data->update_lock);
	tmp = lm85_read_value(client, ADT7463_REG_FANPPR);
	mutex_unlock(&data->update_lock);

	tmp &= ~(GENMASK(nr * 2 + 1, nr * 2));
	tmp |= (val & 0x3) << (nr * 2);

	mutex_lock(&data->update_lock);
	lm85_write_value(client, ADT7463_REG_FANPPR, tmp);
	mutex_unlock(&data->update_lock);
	return count;
}

#define fanppr_reg(offset)						\
static SENSOR_DEVICE_ATTR(adt7463_fanppr##offset##_val,			\
		S_IRUGO | S_IWUSR, show_fanppr, set_fanppr, offset);
...
/* ADT7463_REG_CFG3 nTHERM bit */

static ssize_t show_ntherm(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int value = 0;
	...
	mutex_lock(&data->update_lock);
	value = lm85_read_value(client, ADT7463_REG_CFG3);
	mutex_unlock(&data->update_lock);

	return sprintf(buf, "%d\n", (value >> 1) & 0x1);
}

static ssize_t set_ntherm(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct lm85_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	mutex_lock(&data->update_lock);
	tmp = lm85_read_value(client, ADT7463_REG_CFG3);
	mutex_unlock(&data->update_lock);
	...
	mutex_lock(&data->update_lock);
	lm85_write_value(client, ADT7463_REG_CFG3, tmp);
	mutex_unlock(&data->update_lock);
	return count;
}

static SENSOR_DEVICE_ATTR(adt7463_ntherm_val,
		S_IRUGO | S_IWUSR, show_ntherm, set_ntherm, 0);

#define ADT7463_REG_CFG4 0x7d

static ssize_t show_cfgr4(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int value = 0;
	...
	mutex_lock(&data->update_lock);
	value = lm85_read_value(client, ADT7463_REG_CFG4);
	mutex_unlock(&data->update_lock);

	return sprintf(buf, "%d\n", value);
}

static ssize_t set_cfgr4(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct lm85_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	mutex_lock(&data->update_lock);
	lm85_write_value(client, ADT7463_REG_CFG4, val & 0xf);
	mutex_unlock(&data->update_lock);
	return count;
}

static SENSOR_DEVICE_ATTR(adt7463_cfgr4_reg, S_IRUGO | S_IWUSR,
					show_cfgr4, set_cfgr4, 0);
...
static struct attribute *lm85_attributes[] = {
	...
	&sensor_dev_attr_fan1_measure.dev_attr.attr,
	&sensor_dev_attr_fan2_measure.dev_attr.attr,
	&sensor_dev_attr_fan3_measure.dev_attr.attr,
	&sensor_dev_attr_fan4_measure.dev_attr.attr,
...
static struct attribute *adt7463_specific_attributes[] = {
	&sensor_dev_attr_adt7463_ain0_val.dev_attr.attr,
	&sensor_dev_attr_adt7463_ain1_val.dev_attr.attr,
	&sensor_dev_attr_adt7463_ain2_val.dev_attr.attr,
	&sensor_dev_attr_adt7463_ain3_val.dev_attr.attr,

	&sensor_dev_attr_adt7463_fanppr0_val.dev_attr.attr,
	&sensor_dev_attr_adt7463_fanppr1_val.dev_attr.attr,
	&sensor_dev_attr_adt7463_fanppr2_val.dev_attr.attr,
	&sensor_dev_attr_adt7463_fanppr3_val.dev_attr.attr,

	&sensor_dev_attr_adt7463_cfgr4_reg.dev_attr.attr,

	&sensor_dev_attr_adt7463_ntherm_val.dev_attr.attr,
	NULL
};

static const struct attribute_group adt7463_specific_group = {
	.attrs = adt7463_specific_attributes,
};
...
static int lm85_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (data->type == adt7463)
		data->groups[idx++] = &adt7463_specific_group;
...
MODULE_DESCRIPTION("LM85-B, LM85-C driver (with ADT6374 specific registers)");
