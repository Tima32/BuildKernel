...
#if 0
static int sif_spi_write_then_read(struct spi_device *spi, unsigned char* bufwrite, int sizesend, unsigned char* bufread, int sizeread)
{
	int error;
...
static int sif_spi_write_async_read(struct spi_device *spi, unsigned char* bufwrite,unsigned char* bufread,int size)
{
...
static int sif_spi_write_raw(struct spi_device *spi, unsigned char* buf, int size)
{
	int err;
	struct spi_transfer xfer = {
		.tx_buf		= buf,
...
static int sif_spi_read_raw(struct spi_device *spi, unsigned char* buf, int size)
{
...
static int sif_spi_write_blocks(struct spi_device *spi, unsigned int addr,unsigned char *src, int count)
{
    int err_ret = 0;
    int i,j;
    int n;
...
static int sif_spi_write_mix_sync(struct spi_device *spi, unsigned int addr, unsigned char *buf, int len, int dummymode)
{
	int err;
...
static int sif_spi_read_blocks(struct spi_device *spi, unsigned int addr, unsigned char *dst, int count)
{
    int err_ret = 0;
    int pos,len;
    int i,j;
...
static int sif_spi_read_mix_sync(struct spi_device *spi, unsigned int addr, unsigned char *buf, int len, int dummymode)
{
	int err;
...
static int sif_spi_protocol_init(struct spi_device *spi)
{
...
{
	struct esp_pub *epub = (struct esp_pub *)data;
	struct spi_device *spi = ((struct esp_spi_ctrl *)epub->sif)->spi;

	do {
		sif_dsr(spi);
		
		set_current_state(TASK_INTERRUPTIBLE);
		sif_platform_irq_mask(epub, 0);
...
static int sif_setup_irq_thread(struct esp_pub *epub)
{
	sif_irq_thread = kthread_run(spi_irq_thread, epub, "kspiirqd/eagle");
...
static irqreturn_t sif_irq_handler(int irq, void *dev)
{
	struct spi_device *spi = dev;
	struct esp_spi_ctrl *sif = spi_get_drvdata(spi);
	struct esp_pub *epub = sif->epub;

	sif_platform_irq_mask(epub, 1);

	if (sif_platform_is_irq_occur(epub)) {
...
void sif_enable_irq(struct esp_pub *epub)
{
	...
	sif_platform_irq_init(epub);
	err = sif_setup_irq_thread(epub);
        if (err) {
...
/******************compat with other device in some shared irq system ********************/
        ...
        err = request_irq(sif_platform_get_irq_no(epub), sif_irq_handler, IRQF_TRIGGER_RISING | IRQF_SHARED, "esp_spi_irq", spi);
  #elif defined(REQUEST_IRQ_FALLING)
        err = request_irq(sif_platform_get_irq_no(epub), sif_irq_handler, IRQF_TRIGGER_FALLING | IRQF_SHARED, "esp_spi_irq", spi);
  #elif defined(REQUEST_IRQ_LOWLEVEL)
        err = request_irq(sif_platform_get_irq_no(epub), sif_irq_handler, IRQF_TRIGGER_LOW | IRQF_SHARED, "esp_spi_irq", spi);
  #elif defined(REQUEST_IRQ_HIGHLEVEL)
        err = request_irq(sif_platform_get_irq_no(epub), sif_irq_handler, IRQF_TRIGGER_HIGH | IRQF_SHARED, "esp_spi_irq", spi);
  #else   /* default */
        err = request_irq(sif_platform_get_irq_no(epub), sif_irq_handler, IRQF_TRIGGER_LOW | IRQF_SHARED, "esp_spi_irq", spi);
  #endif /* TRIGGER MODE */
#else
        err = request_irq(sif_platform_get_irq_no(epub), sif_irq_handler, IRQF_TRIGGER_LOW, "esp_spi_irq", spi);
...
#ifdef IRQ_WAKE_HOST
	enable_irq_wake(sif_platform_get_irq_no(epub));
#endif
...
void sif_disable_irq(struct esp_pub *epub)
{
	...
	if (spi == NULL) {
        ...
        free_irq(sif_platform_get_irq_no(epub), spi);
	kthread_stop(sif_irq_thread);

	sif_platform_irq_deinit(epub);

        atomic_set(&sctrl->irq_installed, 0);
}


static int esp_setup_spi(struct spi_device *spi)
{
#ifndef ESP_PREALLOC
	int retry = 10;
#endif
...
#endif
		if ((check_buf = (unsigned char *)kmalloc(256, GFP_KERNEL))
				== NULL)
			goto _err_check_buf;
...
extern const struct spi_device_id esp_spi_id[];
MODULE_DEVICE_TABLE(spi, esp_spi_id);

extern const struct of_device_id esp_spi_of_match[];
...
#define ESP_WAIT_UP_TIME_MS 11000
	...
	for (i = 0, node = NULL; i < ARRAY_SIZE(esp_spi_of_match) && !node; ++i){
		if (esp_spi_of_match[i].compatible[0])
			node = of_find_compatible_node(NULL, NULL,
					esp_spi_of_match[i].compatible);
	}
	if (!node){
		pr_err("esp8089 (eagle) wireless driver module: can't find device tree node with supported compatible");
		return -ENODEV;
	}
	...
	do {
		...
		err = sif_platform_target_poweron(node);
		if (err) {
			esp_dbg(ESP_DBG_ERROR, "eagle spi powering on failed, error code: %d\n", err);
			goto _fail;
		}
		...
		sif_platform_target_poweroff(node);
	...
	if (down_timeout(&esp_powerup_sem, msecs_to_jiffies(ESP_WAIT_UP_TIME_MS)) == 0 && sif_get_ate_config() == 0) {
		if (sif_sdio_state == ESP_SDIO_STATE_FIRST_NORMAL_EXIT){
