...
#define MII_88E1510_PHY_LED_DEF		0x0410
#define MII_88E1510_PHY_LED_ALLOFF	0x0888
...
#define MII_88E1510_GEN_CTRL_REG_1_MODE_RGMII_COPPER		0x0	/* RGMII to Copper */
#define MII_88E1510_GEN_CTRL_REG_1_MODE_SGMII_COPPER		0x1	/* SGMII to Copper */
#define MII_88E1510_GEN_CTRL_REG_1_MODE_RGMII_1000BASEX		0x2	/* RGMII to 1000BASE-X */
#define MII_88E1510_GEN_CTRL_REG_1_MODE_RGMII_100BASEFX		0x3	/* RGMII to 100BASE-FX */
#define MII_88E1510_GEN_CTRL_REG_1_MODE_RGMII_SGMII		0x4	/* RGMII to SGMII */
#define MII_88E1510_GEN_CTRL_REG_1_MODE_RGMII_AMD_SGMII		0x6	/* RGMII to Auto Media Detect Copper/SGMII */
#define MII_88E1510_GEN_CTRL_REG_1_MODE_RGMII_AMD_1000BASEX	0x7	/* RGMII to Auto Media Detect Copper/1000BaseX/100BASE-FX */

#define MII_88E1510_LED_FUNC_CTRL	0x10
...
static int sfp_module_insert(void *_priv, const struct sfp_eeprom_id *id)
{
	...
	dev_info(dev, "SFP module inserted");

	sfp_parse_support(phydev->sfp_bus, id, supported);
	port = sfp_parse_port(phydev->sfp_bus, id, supported);
	interface = sfp_select_interface(phydev->sfp_bus, supported);

	dev_info(dev, "SFP port = %d", port);
	dev_info(dev, "SFP interface = %s", phy_modes(interface));
	dev_info(dev, "SFP support = %*pbl", __ETHTOOL_LINK_MODE_MASK_NBITS,
			supported);

	switch (interface) {
		case PHY_INTERFACE_MODE_SGMII:
			selected_mode = MII_88E1510_GEN_CTRL_REG_1_MODE_RGMII_SGMII;
			break;
		case PHY_INTERFACE_MODE_1000BASEX:
		default:
			selected_mode = MII_88E1510_GEN_CTRL_REG_1_MODE_RGMII_1000BASEX;
	}

	mutex_lock(&phydev->lock);

	oldpage = marvell_get_set_page(phydev, MII_MARVELL_MODE_PAGE);
	if (oldpage < 0) {
	...
	ctrl_reg = phy_read(phydev, MII_88E1510_GEN_CTRL_REG_1);
	ctrl_reg &= ~MII_88E1510_GEN_CTRL_REG_1_MODE_MASK;
	ctrl_reg |= selected_mode;
	ret = phy_write(phydev, MII_88E1510_GEN_CTRL_REG_1, ctrl_reg);
	if (ret < 0)
		goto error;

	/* PHY reset is necessary after changing MODE[2:0] */
	ctrl_reg |= MII_88E1510_GEN_CTRL_REG_1_RESET;
	ret = phy_write(phydev, MII_88E1510_GEN_CTRL_REG_1, ctrl_reg);
	if (ret < 0)
		goto error;

	ret = marvell_set_page(phydev, MII_MARVELL_LED_PAGE);
	if (ret < 0)
		goto error;
	ret = phy_write(phydev, MII_88E1510_LED_FUNC_CTRL, MII_88E1510_PHY_LED_ALLOFF);
...
static void sfp_module_remove(void *_priv)
{
	...
	dev_info(dev, "SFP module removed");
	...
	ctrl_reg = phy_read(phydev, MII_88E1510_GEN_CTRL_REG_1);
	ctrl_reg &= ~MII_88E1510_GEN_CTRL_REG_1_MODE_MASK;
	ctrl_reg |= MII_88E1510_GEN_CTRL_REG_1_MODE_RGMII_COPPER;
	phy_write(phydev, MII_88E1510_GEN_CTRL_REG_1, ctrl_reg);
	...
	phy_write(phydev, MII_88E1510_LED_FUNC_CTRL, MII_88E1510_PHY_LED_DEF);

	marvell_set_page(phydev, oldpage);
	mutex_unlock(&phydev->lock);
}

static void sfp_link_down(void *_priv)
{
	struct phy_device *phydev = _priv;
	struct device *dev = &phydev->mdio.dev;

	dev_info(dev, "SFP link down");

	marvell_read_status(phydev);
}

static void sfp_link_up(void *_priv)
{
	struct phy_device *phydev = _priv;
	struct device *dev = &phydev->mdio.dev;

	dev_info(dev, "SFP link up");
...
static int m88e1510_probe(struct phy_device *phydev)
{
	...
	err = phy_sfp_probe(phydev, &sfp_phy_ops);
